...
#import "XENPWeatherController.h"
#import "XENPResources.h"

@interface XENPWeatherController ()

@end

@implementation XENPWeatherController

-(id)specifiers {
    if (_specifiers == nil) {
        NSMutableArray *testingSpecs;
        if ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad))
            testingSpecs = [self loadSpecifiersFromPlistName:@"WeatheriPad" target:self];
        else
            testingSpecs = [self loadSpecifiersFromPlistName:@"Weather" target:self];
