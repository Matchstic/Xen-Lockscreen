...
-(id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        NSString *imagePath = [NSString stringWithFormat:@"/Library/PreferenceBundles/XenPrefs.bundle/Background%@", [XENPResources imageSuffix]];
        ...
        NSMutableAttributedString* attrStr2 = [[NSMutableAttributedString alloc] initWithString:@"LOCKSCREEN" attributes:
        ...
        self.toggle = [[XENPHeaderSwitch alloc] initWithFrame:CGRectZero andCallback:^{
            // Update settings!
            [XENPResources setPreferenceKey:@"enabled" withValue:[NSNumber numberWithBool:[self.toggle currentValue]]];
        }];
        
        self.toggle.backgroundColor = [UIColor clearColor];
        [self.toggle setOn:([XENPResources getPreferenceKey:@"enabled"] ? [[XENPResources getPreferenceKey:@"enabled"] boolValue] : YES) animated:NO];
        
        [self.container addSubview:self.toggle];
    }
...
-(void)layoutSubviews {
    ...
    //self.tweakName.frame = CGRectMake(0, 0, self.bounds.size.width, self.tweakName.frame.size.height);
    ...
    self.toggle.center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2 + self.toggle.frame.size.height*1.15 + self.tweakSubtitle.frame.size.height - 10);
