...
static XENSetupWindow *shared;

@implementation XENSetupWindow

+(instancetype)sharedInstance {
    if (!shared) {
        static dispatch_once_t p = 0;
        dispatch_once(&p, ^{
            shared = [[XENSetupWindow alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
        });
    }
...
+(void)finishSetupMode {
    ...
    [UIView animateWithDuration:0.5 delay:0.3 options:UIViewAnimationCurveEaseOut animations:^{
        ...
        if (finished) {
            shared = nil;
            [(SpringBoard*)[UIApplication sharedApplication] _xen_finaliseAfterSetup];
            [(SpringBoard*)[UIApplication sharedApplication] _xen_releaseSetupUI];
        }
    }];
}

+(void)relayoutXenForSetupFinished {
    [(SpringBoard*)[UIApplication sharedApplication] _xen_relayoutAfterSetupContentEditorDisplayed];
}
...
-(id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.windowLevel = 1080;
        
        self.usingQuickSetup = NO;
        
        // We want a navigation controller
        //UDLRSetupHello *hello = [[UDLRSetupHello alloc] init];
        XENSetupInitialController *initial = [[XENSetupInitialController alloc] init];
